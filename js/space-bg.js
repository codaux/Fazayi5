// Decorative space background (no UI buttons)
// Adapted from codepen sample, trimmed for background-only usage.
...
class Effect {
  ...
  threeInit() {
    ...
    // OrbitControls auto-rotate only; interactions are disabled by CSS pointer-events none
  ...
  createElements() {
    ...
    const materialSphereBg = new THREE.MeshBasicMaterial({ side: THREE.BackSide });
  ...
  createPointElement() {
    ...
    function createPointParticles({ texture, size, total, transparent = true, max = 150, min = 70, pointY }) {
      ...
      const pointMaterial = new THREE.PointsMaterial({ size, blending, transparent: true, depthWrite: false });
  ...
  createMovingStars() {
    ...
    const starsMaterial = new THREE.PointsMaterial({ size: 14, transparent: true, opacity: 0.8, blending: THREE.AdditiveBlending, depthWrite: false });
  ...
  updateMovingStars() {
    ...
    const movingStarsEasing = Math.min(1, (Date.now() - animationStartTime6) / 2000);
